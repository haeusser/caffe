...
void HuberL1LossLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
                                         const vector<Blob<Dtype>*>& top) 
{
  ...
  if(bottom.size() == 1) {
    ...
    diff_layer_->SetUp(bottom, diff_top_vec_);
      ...
      LOG(FATAL) << "HuberL1LossLayer needs one or two input blobs.";
  }
  ...
  if (this->layer_param_.huber_l1_loss_param().l2_per_location()) { 
    ...
    if(this->layer_param_.huber_l1_loss_param().l2_prescale_by_channels()) {
    ...
    sqrt_param.mutable_power_param()->set_shift(this->layer_param_.huber_l1_loss_param().epsilon());
...
template <typename Dtype>
void HuberL1LossLayer<Dtype>::Reshape(
...
template <typename Dtype>
void HuberL1LossLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
                                          const vector<Blob<Dtype>*>& top) 
{
  NOT_IMPLEMENTED;
}
...
template <typename Dtype>
void HuberL1LossLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
#ifdef CPU_ONLY
STUB_GPU(HuberL1LossLayer);
#endif

INSTANTIATE_CLASS(HuberL1LossLayer);
REGISTER_LAYER_CLASS(HuberL1Loss);
