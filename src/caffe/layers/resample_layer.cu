        ...
        const bool antialias)
{
    ...
    {
        int x_out=index % out_width;
        int y_out=index / out_width;
        ...
        for(int y=y_in_round-rx; y<=y_in_round+rx; y++)
            for(int x=x_in_round-ry; x<=x_in_round+ry; x++)
            {
                ...
                sum += w * in_ptr[y*in_width+x];
                wsum += w;
            }

        out_ptr[y_out*out_width+x_out] = (!wsum) ? 0 : (sum / wsum);
    }
}
...
void ResampleLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  if(this->layer_param().resample_param().type() == ResampleParameter_ResampleType_CUBIC)
      ...
      LOG(FATAL) << "only BICUBIC and LINEAR resampling is supported for now. ";
