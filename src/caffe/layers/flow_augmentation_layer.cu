...
__global__ void WarpData(const int nthreads, const int num, const int height, const int width, const Dtype* src_data, const int src_count,
  ...
  CUDA_KERNEL_LOOP(index, nthreads) {
    ...
    // / 0 2 4 \
    // \ 1 3 5 /
    const typename PhilDataAugmentationLayer<Dtype>::tTransMat *transMat1 = &(transMats1[n]);
    const typename PhilDataAugmentationLayer<Dtype>::tTransMat *transMat2 = &(transMats2[n]);
