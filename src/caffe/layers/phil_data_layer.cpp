...
void PhilDataLayer<Dtype>::generateRandomPermutation(int seed, int block_size) {
  if (seed > 0)
...
void* PhilDataLayerPrefetch(void* layer_pointer) {
  CHECK(layer_pointer);
  PhilDataLayer<Dtype>* layer = static_cast<PhilDataLayer<Dtype>*>(layer_pointer);
  CHECK(layer);
  Datum datum;
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    case DataParameter_DB_LEVELDB:
      LOG(FATAL) << "LevelDB not supported by PhilData";
      break;
...
template <typename Dtype>
PhilDataLayer<Dtype>::~PhilDataLayer<Dtype>() {
  JoinPrefetchThread();
...
template <typename Dtype>
void PhilDataLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
...
void PhilDataLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  if (top.size() == slice_point_.size() + 1) {
    ...
    LOG(FATAL) << "PhilDataLayer has " << top.size() << " top blobs, but " << (slice_point_.size()+1) << " slices."; 
  }
  ...
  if (this->layer_param_.data_param().rand_skip()) {
        LOG(FATAL) << "No rand_skip for PhilData layer";
  }
...
template <typename Dtype>
void PhilDataLayer<Dtype>::CreatePrefetchThread() {
  ...
  // Create the thread.
  CHECK(!pthread_create(&thread_, NULL, PhilDataLayerPrefetch<Dtype>,
        static_cast<void*>(this))) << "Pthread execution failed.";
}

template <typename Dtype>
void PhilDataLayer<Dtype>::JoinPrefetchThread() {
  CHECK(!pthread_join(thread_, NULL)) << "Pthread joining failed.";
}

template <typename Dtype>
unsigned int PhilDataLayer<Dtype>::PrefetchRand() {
  CHECK(prefetch_rng_);
...
template <typename Dtype>
void PhilDataLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {  
...
#ifdef CPU_ONLY
STUB_GPU_FORWARD(PhilDataLayer, Forward);
#endif

INSTANTIATE_CLASS(PhilDataLayer);
REGISTER_LAYER_CLASS(PhilData);
