...
template <typename Dtype>
class KittiErrorLayer : public LossLayer<Dtype> {
 public:
  explicit KittiErrorLayer(const LayerParameter& param)
  ...
  virtual inline const char* type() const { return "KittiError"; }
...
class HuberL1LossLayer : public LossLayer<Dtype> {
 public:
  explicit HuberL1LossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param), sign_() {}
  virtual void LayerSetUp(
        const vector<Blob<Dtype>*>& bottom,
        const vector<Blob<Dtype>*>& top);    
