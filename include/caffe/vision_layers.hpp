...
/**
 * 
 *  Tolerance layer by Phil
 *  Input: Original Ground Truth and Prediction
 *  Output: Spatially tolerant Ground Truth to be used with loss layer
 * 
 **/

template <typename Dtype>
class ToleranceLayer : public Layer<Dtype> {
 public:
  explicit ToleranceLayer(const LayerParameter& param)
      : Layer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  virtual inline const char* type() const { return "Tolerance"; }
  virtual inline int ExactNumBottomBlobs() const { return 2; }
  virtual inline int ExactNumTopBlobs() const { return 1; }
