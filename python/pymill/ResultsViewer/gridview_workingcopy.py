...
def format_template(index, suffix):
  if '%' in suffix:
    return suffix%('%07d'%index)
  else:
    return filename_template%(index, suffix)

def format_template_batch(index_a, index_b, suffix):
  print(suffix)
  if '%' in suffix:
    return suffix%('%07d(%03d)'%(index_a, index_b))
  else:
    return filename_template_batch%(index_a, index_b, suffix)
...
def processSuffix(s):
  ...
  if s.startswith('DIFF'):
    ...
    import convert_workingcopy as convert
    a = m.groupdict()['a']
    b = m.groupdict()['b']
    consistency_check(a, b)
    #testfile = filename_template_batch%(0,0,s)
    testfile = format_template_batch(0,0,a)
    ...
    consistency_check(a, b)
    #testfile = filename_template_batch%(0,0,s)
    testfile = format_template_batch(0,0,s)
    batch = os.path.isfile(os.path.join(dir, testfile))
  ...
  if batch:
    ...
    while True:
      ...
      for (idx,suffix) in enumerate(suffices):
        ...
        f = os.path.join(dir, format_template_batch(i,j,suffix))
        if os.path.isfile(f):
          found[idx].append(f)
          valid_index = True
    ...
    while True:
      ...
      for (idx,suffix) in enumerate(suffices):
        ...
        f = os.path.join(dir, format_template(i,suffix))
        if os.path.isfile(f):
          found[idx].append(f)
          valid_index = True
...
class MainWindow(QtWidgets.QMainWindow):
  def __init__(self, parent=None):
    ...
    testfile = format_template_batch(0,0,self.grid[0].suffix)
    global batch
    batch = os.path.isfile(os.path.join(dir, testfile))
